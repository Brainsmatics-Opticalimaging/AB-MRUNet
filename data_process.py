    ...
    for i, file in enumerate(file_list):
        if name == "DRIVE":
            ...
            if not file.endswith("gz"):
                ...
                img_list.append(ToTensor()(img))
                gt_list.append(ToTensor()(gt))
    img_list = normalization1(img_list)

    if mode == "training":
        img_list = datastrong(img_list)
        gt_list = datastrong(gt_list)
...
def datastrong(imgs_list):
    strong_list=[]
    for img in imgs_list:
       # img1=torch.flip(img,dims=(1,))
       # img2 = torch.flip(img, dims=(2,))

        img2_x = F.interpolate(img, scale_factor=0.5)
        img1 = torch.flip(img2_x, dims=(1,))
        img2 = torch.flip(img2_x, dims=(2,))

        img4_x=F.interpolate(img, scale_factor=0.25)
        img3 = torch.flip(img4_x, dims=(1,))
        img4 = torch.flip(img4_x, dims=(2,))
        img5 = torch.flip(img3, dims=(2,))

        img8_x = F.interpolate(img, scale_factor=0.125)
        img6 = torch.flip(img8_x, dims=(1,))
        img7 = torch.flip(img8_x, dims=(2,))
        img8 = torch.flip(img6, dims=(2,))

        strong_list.append(img)
        strong_list.append(img1)
        strong_list.append(img2)

        strong_list.append(img4_x)
        strong_list.append(img3)
        strong_list.append(img4)
        strong_list.append(img5)

        strong_list.append(img8_x)
        strong_list.append(img6)
        strong_list.append(img7)
        strong_list.append(img8)
    return strong_list

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-dp', '--dataset_path', default="DATA_8x", type=str,
    ...
    parser.add_argument('-s', '--stride', default=64,
